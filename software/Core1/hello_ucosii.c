...
// Short 3-note ascending chime — ~180ms total, non-intrusive
void play_chime(void) {
    IOWR_ALTERA_AVALON_PIO_DATA(BUZZER_BASE, NOTE_CS5);  OSTimeDlyHMSM(0,0,0,50);
    IOWR_ALTERA_AVALON_PIO_DATA(BUZZER_BASE, NOTE_AS4);  OSTimeDlyHMSM(0,0,0,80);
    IOWR_ALTERA_AVALON_PIO_DATA(BUZZER_BASE, NOTE_DS5);  OSTimeDlyHMSM(0,0,0,80);
    IOWR_ALTERA_AVALON_PIO_DATA(BUZZER_BASE, NOTE_C5);  OSTimeDlyHMSM(0,0,0,80);
...
void accel_task(void* pdata) {
    ...
    int emergency_led = 0;
    while (1) {
        OSTimeDly(1);  // 1 tick — yields to higher-priority tasks, then immediately returns
        ...
        // Signal Core 0 that fresh accel data is ready
...
void ui_task(void* pdata) {
    ...
    int     led_tick_counter = 0; // Software timing register for animations
    ...
    while (1) {
        ...
        if (chime_flag && !emergency_stop) {
                    chime_flag = 0;
                    play_chime();   // ~180ms — runs here in UI task context, safe to delay
                }

                led_tick_counter++;
                uint8_t ext_led_mask = 0;

                if (emergency_stop) {
                    // Emergency Lockout: Flash RED (Bit 0) aggressively, force others off
                    if ((led_tick_counter % 4) < 2) {
                        ext_led_mask |= 0x4; // RED ON
                    }
                } else {
                    // FIXED: GREEN (Bit 1) is now a steady, solid ON to eliminate flashing irritation
                    ext_led_mask |= 0x2; // GREEN SOLID ON

                    // Bus Streaming telemetry: Flicker YELLOW (Bit 2) dynamically during SPI bursts
                    if (start_spi) {
                        if ((led_tick_counter % 2) == 0) {
                            ext_led_mask |= 0x1; // YELLOW ON
                        }
                    }
                }
                // Commit live states directly into the 3-bit physical port
                IOWR_ALTERA_AVALON_PIO_DATA(EXTERN_LEDS_BASE, ext_led_mask);
                // ─────────────────────────────────────────────────────────────────────
        // Commit live states directly into the 3-bit physical port
        IOWR_ALTERA_AVALON_PIO_DATA(EXTERN_LEDS_BASE, ext_led_mask);
        // ─────────────────────────────────────────────────────────────────────
